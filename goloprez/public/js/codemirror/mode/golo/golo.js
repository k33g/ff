 ...
 */
CodeMirror.defineMode('golo', function(conf) {
    ...
    function tokenLexer(stream, state) {
        ...
        if (state.dedent > 0 && stream.eol() && state.scopes[0].type == 'golo') {
    ...
    var external = {
        startState: function(basecolumn) {
            return {
                tokenize: tokenBase,
                scopes: [{offset:basecolumn || 0, type:'golo'}],
                lastToken: null,
                lambda: false,
                dedent: 0
            };
...
CodeMirror.defineMIME('text/x-golo', 'golo');
